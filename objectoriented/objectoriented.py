...
class Candidate:
    def __init__(self, val, pos_score, neg_score):
        self.v = val
        self.p = pos
        self.n = neg
    ...
    def check_plus_pos(self,guessval):
        ...
        for i in range(len(sv)):
            if sv[i] == gv[i]:
                plus += 1
        return plus
    def check_plus_neg(self,guessval):
        val = self.v
        sv = str(val)
        gv = str(guessval)
        neg = 0
    ...
    def consistent(self, guess):
        if (self.check_plus_pos(guess.v)) == (guess.p) and (self.check_plus_neg(guess.v)) == (guess.n)
            return True
        else:
            return False